# Project Singularity — SOP 操作手册（初稿）

> 本手册覆盖 AIGC 微短剧从角色出图到最终成片的完整工业化流程。  
> 以 Project Singularity /《奇点回响》为例演示，所有角色、剧情、参数可按实际项目替换。
...
2. 将角色圣经保存至 `02_Scripts/角色圣经_艾娃.md`
...
4. 设置参数：
   ...
   - CFG：3.5–4.5
   - 使用 FP8 精度以节省显存
5. 批量生成 20 张关键帧，覆盖核心场景
6. 盲测筛选：将图片随机打乱，由导演/美术判断是否认定为同一人

### 2.3 输出检查

- [ ] 角色三视图已保存至 `01_Assets/Characters/艾娃/`
- [ ] 20 张关键帧已保存至 `01_Assets/Scenes/`
- [ ] 盲测通过（团队一致认为不"变脸"）
...
1. 全平台上传正片
2. 技术社区发布工作流教程
3. 收集反馈并归档至 `06_Research/反馈迭代.md`
...
例：PS_S01_03_v02_AvaCloseUp.png
```
...
> 本 SOP 随项目推进持续更新。任何流程优化请在 `04_SOP/更新记录.md` 中登记。

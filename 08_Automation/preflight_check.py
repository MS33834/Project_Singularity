...
# 需要检查的 Python 包
REQUIRED_PACKAGES = ["torch", "requests", "PIL"]
        ...
        with open("/proc/meminfo", "r") as f:
            for line in f:
                if line.startswith("MemTotal:"):
                    ...
                    return
        result.warning("无法读取内存信息")
    except Exception:
        result.warning("无法检查内存（非 Linux 系统？）")
...
def check_packages(result: CheckResult):
    ...
    for pkg in REQUIRED_PACKAGES:
        try:
            __import__(pkg)
            result.ok(f"Python 包: {pkg}")
        except ImportError:
            result.error(f"Python 包未安装: {pkg}（pip install {pkg}）")

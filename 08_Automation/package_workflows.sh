#!/usr/bin/env bash
# -*- coding: utf-8 -*-
# Project Singularity — ComfyUI 工作流打包脚本
# 用法: bash 08_Automation/package_workflows.sh [版本号]
#
# 功能:
#   1. 收集 03_Workflows/ 下的 JSON 工作流与节点依赖说明
#   2. 生成打包清单 README
#   3. 输出 ZIP 到 05_Output/Final/workflows/Project_Singularity_Workflows_v{版本}.zip
...
VERSION="${1:-1.0}"
ZIP_NAME="Project_Singularity_Workflows_v${VERSION}.zip"
TMP_DIR="$(mktemp -d)"
...
# 2. 复制节点依赖说明
echo "[2/4] 复制节点依赖说明..."
cp "$WORKFLOWS_DIR/节点依赖.md" "$TMP_DIR/workflows/" 2>/dev/null || true
cp "$WORKFLOWS_DIR/ComfyUI节点连接说明.md" "$TMP_DIR/workflows/" 2>/dev/null || true
cp "$WORKFLOWS_DIR/README.md" "$TMP_DIR/workflows/" 2>/dev/null || true

# 3. 生成打包清单
echo "[3/4] 生成打包清单..."
cat > "$TMP_DIR/workflows/README_Packaged_Workflows.md" << EOF
# Project Singularity — ComfyUI 工作流打包清单
...
| ComfyUI节点连接说明.md | 节点连接与参数说明 |
| 节点依赖.md | 所需自定义节点与模型列表 |

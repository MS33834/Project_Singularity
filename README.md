...
本仓库不是一部已经拍完的影片，而是一条**完整的 AIGC 工业化生产经验链**。它展示的是：

- 如何从 0 到 1 策划一部 AIGC 短片；
- 如何用 DeepSeek/Claude 产出剧本与角色圣经；
- 如何用 Flux.1 Kontext + IPAdapter 解决角色一致性难题；
- 如何用 Wan2.2 双专家模型生成稳定视频；
- 如何用可灵 2.5 Turbo 处理复杂镜头；
- 如何用 ElevenLabs / Suno / 达芬奇 / Topaz 完成后期与音效；
- 如何用 ComfyUI 工作流、自动化脚本、QA 检测、双仓库同步实现工业化协作。

如果你需要向别人证明自己有 AIGC 项目经验，可以直接把本仓库作为**作品集**：它包含计划书、分镜、工作流、SOP、脚本、进度管理、风险管控、发布模板等完整链路。详细说明见 [`AIGC_Experience_Chain.md`](./AIGC_Experience_Chain.md)。
...
├── 05_Output/              # 输出成片
│   ├── Rough_Cuts/         # 粗剪版本与锁定剪辑
│   ├── EDL/                # 编辑决策列表
│   └── Final/              # 最终成片、交付规格、调色/混音记录
├── 06_Research/            # 技术栈、预算、授权合规、备份方案、发布计划、调优、失败案例
├── 07_Team/                # 专家名册、任务分派、启动会
├── 08_Automation/          # 部署脚本、API 调用、基准测试、工作流打包
├── 09_Release/             # 发布检查清单、教程模板、答辩材料
...
├── 项目总结报告_模板.md     # 结项归档模板
├── 周报模板.md              # 每周师生同步会汇报模板
├── .env.example             # API 密钥与环境变量模板（不提交真实密钥）
├── .gitattributes           # Git 大文件/换行符规则
└── .gitignore               # 忽略 .env、模型、输出等敏感/大文件
```
...
2. 输出 24 个原始视频片段（含重拍/变体可达 40-60）

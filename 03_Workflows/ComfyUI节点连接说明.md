...
> - `03_Workflows/节点依赖.md` — 节点与模型依赖清单
>
...
> 说明：`weight` 0.7–0.8 为本项目调优区间（见 `06_Research/参数调优记录表.md`）。过低一致性不足，过高易出现参考图痕迹/过拟合。
...
| 11 | SaveImage | prefix=PS_Ava_ | VAEDecode.IMAGE | — |
...
|------|------|
| 节点类型 | `LoadImage` |
| 关键参数 | `image`: 选择由 Flux 工作流生成的关键帧（如 `PS_Ava_S01_03.png`） |
| 输入连接 | 无（根节点） |
...
| 关键参数 | `filename_prefix`: `PS_Shot_`；`fps`: `24`；`format`: `video/h264-mp4`（或 `webp`/`gif`）；`save_metadata`: `true` |
...
| 9 | VideoCombine | prefix=PS_Shot_, fps24, mp4 | VAEDecode(Video).IMAGE | — |
...
### Flux 角色一致性（参考 `06_Research/参数调优记录表.md`）
...
> 本文档随工作流实际搭建与调优持续更新。重大节点/参数变更请在 `06_Research/参数调优记录表.md` 同步登记。
